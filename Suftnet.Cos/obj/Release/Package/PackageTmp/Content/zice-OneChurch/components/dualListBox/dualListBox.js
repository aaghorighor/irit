...
(function ($) {
    ...
    $.configureBoxes = function (options) {
        ...
        if (IsMoveMode(settings[index])) {
            ...
            $('#' + settings[index].to1).click(function () {              
                $('#' + group2[index].view + ' option:selected').remove();
            });
        ...
        $('#' + settings[index].to2).click(function () {
            ...
            var selected = true;           
            var $fromoption = $('#' + group1[index].view + ' option:selected');          
            $('#' + group2[index].view).children("option").each(function () {
                var $this = $(this);
                if ($fromoption.val() == $this.val()) {
                    selected = false;                 
                }
            });

            if (selected == true) {
                $('#' + group2[index].view).append('<option selected=selected value="' + $fromoption.val() + '">' + $fromoption.text() + '</option>');
                Filter(group2[index]);
                if (settings[group2[index].index].useCounters) { UpdateLabel(group2[index]); }
            }
        ...
        $('#' + settings[index].allTo2).click(function () {
            ...
            $('#' + group2[index].view + ' option').remove();
            $('#' + group1[index].view).children("option").each(function () {
                var $this1 = $(this);                
                $('#' + group2[index].view).append('<option value="' + $this1.val() + '">' + $this1.text() + '</option>');                 
            });
    ...
    function IsMoveMode(currSettings) {       
        return currSettings.transferMode == 'copy';
    }
})(jQuery);